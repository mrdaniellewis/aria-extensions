...
ariaExtensions[extend](Element.prototype, 'role', { get() {
  return Aria.create(this).role;
} });

ariaExtensions[extend](Element.prototype, 'aria', { get() {
  return Aria.create(this);
} });

ariaExtensions[extend](Element.prototype, 'hasRole', { value(roles, options) {
...
ariaExtensions[extend](Element.prototype, 'closestRole', { value(roles, options) {
...
ariaExtensions[extend](Element.prototype, 'findRole', { value(roles, { exact = false } = {}) {
  if (exact) {
    roles = [].concat(roles);
  } else {
...
ariaExtensions.expandWithChildRoles = function expandWithChildRoles(names) {
  ...
  const found = [...new Set(names.concat(...names.map(name => this.expandWithChildRoles(
