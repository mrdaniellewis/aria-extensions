...
function blendWithBackground(colour, el) {
  ...
  do {
    ...
    let opacity = 1;
    if (cursor !== document) {
      background = colourParts(cursor[symbol('style')]('backgroundColor'));
      opacity = cursor[symbol('style')]('opacity');
    }
    if (background[3] !== 0) {
...
function textColour(el) {
  const colour = colourParts(el[symbol('style')]('color'));
...
ariaExtensions[symbol('extend')](HTMLElement.prototype, 'contrast', {
  get() {
...
ariaExtensions[symbol('extend')](HTMLElement.prototype, 'textColour', {
  get() {
    return textColour(this);
  },
});

ariaExtensions[symbol('extend')](HTMLElement.prototype, 'backgroundColour', {
  get() {
    return backgroundColour(this);
  },
});
...
ariaExtensions[symbol('blend')] = blend;
ariaExtensions[symbol('luminosity')] = luminosity;
ariaExtensions[symbol('colourParts')] = colourParts;
ariaExtensions[symbol('contrastRatio')] = contrastRatio;
